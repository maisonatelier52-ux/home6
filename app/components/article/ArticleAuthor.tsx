import React from 'react';
import Link from 'next/link';
import Image from 'next/image';

interface ArticleAuthorProps {
    name: string;
    bio: string;
    image?: string;
    role: string;
}

export default function ArticleAuthor({ name, bio, image, role }: ArticleAuthorProps) {
    return (
        <div className="flex flex-col items-start gap-6 border-y border-slate-300 py-9 md:flex-row md:items-center">
            {/* Author Image */}
            {image && <div className="flex-shrink-0">
                <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-gray-100 relative group">
                    <Image
                        src={image}
                        alt={name}
                        fill
                        sizes="(max-width: 768px) 96px, 128px"
                        className="object-cover group-hover:grayscale-0 transition-all duration-300"
                    />
                </div>
            </div>}

            {/* Author Info */}
            <div className="flex-1 border-l-none md:border-l border-gray-300 pl-4">
                <div className="flex items-center gap-2 mb-2">
                    <span className="text-gray-400 text-xs font-bold tracking-wider">{role}</span>
                </div>

                <h2 className="text-2xl font-serif font-bold text-gray-900 mb-3 cursor-pointer hover:text-red-700 transition-colors">
                    <Link href={`/authors#${(name || '').toLowerCase().replace(/\s+/g, '-')}`} title={`View all posts by ${name}`}>
                        {name}
                    </Link>
                </h2>

                <p className="text-gray-600 font-serif text-sm text-base leading-relaxed mb-4">
                    {bio}
                </p>
            </div>
        </div>
    );
}
